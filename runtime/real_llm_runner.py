...
def run_tasks(tasks: list[SelectedTask], backend: Any, model: str, output_dir: Path, *, seed: int = 42, max_tokens: int = 256, memory=None, state_metrics=None) -> dict[str, Any]:
    ...
    for index, task in enumerate(tasks, 1):
        ...
        try:
            ...
            if state_metrics:
                ...
                state=StateVector(task_phase='generation',agent_role='testgen',progress_flags=('request_started',),memory_reference_ids=tuple(r.memory_id for r in prior))
                encoded=state.stable();state_metrics.count+=1;state_metrics.valid+=1;state_metrics.serialized_bytes+=len(encoded);state_metrics.updates+=1;state_metrics.transitions+=1
            ...
            if memory: memory.write(source_task_id=task.source_task_id,source_round_index=index,category='public_test_strategy',key=task.group_id,safe_summary=f"Public strategy for {task.function_name}",tags=(task.group_id,))
        except Exception as error:
            record.update({"error_type": getattr(error, "error_type", type(error).__name__), "parse_result": "backend_error"})
        records.append(record)
    summary = {"result_scope": RESULT_SCOPE, "conclusion_scope": CONCLUSION_SCOPE, "planned_rounds": len(tasks), "executed_rounds": len(records), "succeeded_rounds": sum(r["status"] == "succeeded" for r in records), "failed_rounds": sum(r["status"] == "failed" for r in records), "skipped_rounds": 0, "memory": memory.trace() if memory else {"enabled":False},"state":state_metrics.trace() if state_metrics else {"state_enabled":False}}
