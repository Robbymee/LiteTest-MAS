...
def execute_task(root,item,out,backend):
 ...
 key=task_key(item);pub=Path(out)/'public'/'tasks';priv=Path(out)/'private';attempts=priv/'attempts';attempts.mkdir(parents=True,exist_ok=True);attempt_no=len(list(attempts.glob(key+'-attempt-*.json')))+1;previous=json.loads((pub/(key+'.json')).read_text(encoding='utf8')) if (pub/(key+'.json')).is_file() else {};resume_count=int(previous.get('resume_count',0))+(1 if previous.get('status')=='running' else 0);atomic_write(attempts/(f'{key}-attempt-{attempt_no}.json'),{'task_id':task.task_id,'attempt':attempt_no,'resume_count':resume_count,'status':'running'});atomic_write(pub/(key+'.json'),{'status':'running','task_id':task.task_id,'attempt_count':attempt_no,'resume_count':resume_count})
