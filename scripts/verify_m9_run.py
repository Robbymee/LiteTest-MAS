...
def verify(run_root, planned, strict=False, expected_count=None):
 ...
 return result
def main():
 p=argparse.ArgumentParser();p.add_argument('--run-root',required=True);p.add_argument('--strict',action='store_true');p.add_argument('--json-output',action='store_true');p.add_argument('--expected-count',type=int);a=p.parse_args();planned=[]
 ...
 result=verify(a.run_root,planned,a.strict,a.expected_count);print(json.dumps(result,sort_keys=True));return 0 if result['valid'] else 1
