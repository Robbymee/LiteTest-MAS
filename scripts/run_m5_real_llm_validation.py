...
def main():
 p=argparse.ArgumentParser(description='M5 real local-LLM integration validation');p.add_argument('--dataset',choices=['mbpp','humaneval'],required=True);p.add_argument('--group-index',type=int,default=0);p.add_argument('--limit',type=int);p.add_argument('--output-dir',required=True);p.add_argument('--seed',type=int,default=42);p.add_argument('--max-tokens',type=int,default=256);p.add_argument('--shared-memory',action='store_true');a=p.parse_args()
 ...
 summary=run_tasks(selected,create_backend(config),config.model,ROOT/a.output_dir,seed=a.seed,max_tokens=a.max_tokens,memory=memory)
