...
def evaluate(input_dir: Path, output_dir: Path, strict: bool=True) -> dict:
    ...
    manifest={'schema_version':'1.0','evaluator_version':'m3','result_scope':'mock_validation','conclusion_scope':'infrastructure_and_accounting_validation_only','source_run_paths':sorted(path.parent.relative_to(input_dir).as_posix() for path in input_dir.rglob('run_summary.json')),'datasets':sorted({run['dataset'] for run in runs}),'modes':sorted({run['mode'] for run in runs}),'seeds':sorted({run['seed'] for run in runs}),'unavailable_metrics':['actual_token_count','estimated_token_count','wall_time_seconds','peak_process_memory_bytes','shared_memory','state_vector']}
    manifest['evaluation_input_sha256']=hashlib.sha256(canonical(runs).encode()).hexdigest()
