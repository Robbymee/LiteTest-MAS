...
当前分支在冻结实验之后修复了 V2 引用执行语义：Protocol 继续只传 StateVector 与 Memory 引用 ID，Executor 现在解析 33-byte StateVector、验证目标角色和阶段，并只注入一次公开状态与已门控的安全 Memory 摘要。该修复未重新运行或修改 M9/M9.1 正式记录；M9.1 报告仍绑定 `c79fd4826627bf61faf5d90540a014d243a59edd`。
...
- openEuler acceptance log: `<openEuler 工作目录>/LiteTest-MAS/runs/validation/m5_openeuler-20260711-074049.log`.
...
- Runner validation commit: `c275a15cd71e76a52ce9fa551916541f410320b5`; openEuler log: `<openEuler 工作目录>/LiteTest-MAS/runs/validation/m2_2_openeuler-20260711-105126.log`.
...
- openEuler log: `<openEuler 工作目录>/LiteTest-MAS/runs/validation/m2_3_openeuler-20260711-110801.log`.
...
P1 已验收。当时的唯一下一步是 P2：基于既有 240 条 M9 公开记录进行无模型调用的指标补充聚合。
...
P3 已完成。新增 `scripts/analyze_m9_task_groups.py`，从正式 `experiments/m9_experiment_spec.json` 读取真实 `dataset`、`group_id`、`task_id` 和 manifest 顺序，从 P2 已生成的公开 CSV 读取 dataset×G1-G4 指标及 G4 Memory 明细，生成 `docs/关联连续任务设计说明.md`。该历史后处理脚本不读取私有运行记录、不发起模型调用，也不修改 M9 数据。
...
Windows P6 专项测试 `4 passed`，全量测试 `76 passed`，`git diff --check` 通过。openEuler P6 专项测试 `4 passed`，全量测试 `75 passed, 1 skipped`，`git diff --check` 通过。两端代码 SHA 为 `af8999a`，openEuler 工作区干净。P6 当时仅使用合成 fixtures；其后续版本已接入 M9.1 冻结实验。本阶段没有运行新的正式实验。
...
Windows P7 专项测试 `5 passed`，全量测试 `79 passed`，`git diff --check` 通过。openEuler P7 专项测试 `5 passed`，全量测试 `78 passed, 1 skipped`，`git diff --check` 通过。两端代码 SHA 为 `c5b971e`，openEuler 工作区干净。P7 当时仅使用合成 fixtures；其后续版本已接入 M9.1 冻结实验。本阶段没有运行新的正式实验。
...
## M9.1 P9.11 正式运行验收失败记录

在 openEuler 的 freeze worktree `87c44618538cb662567e0f629055cb62adfc5910` 中，M9.1 S1-S4 正式 Runner 已执行一次并产生 `planned=240`、`completed=240` 条独立公开记录。Strict Verifier 返回 `valid=true`：任务计划数、最终记录数均为 240，无缺失、无重复，completion marker 与 freeze SHA 和确定性 Spec SHA 一致。对公开 JSON 的递归字段审计未发现 `candidate_code`、`raw_response`、`hidden_reference_tests`、`canonical_solution`、`official_tests`、`expected_output`、凭据或私有回溯字段；M9 原始结果、M9 freeze、Dashboard 和 `v1.0.0-experiment` 未改动。

但运行后只读聚合发现 S1-S4 的通信、StateVector 和 Memory 指标被统一写为 `unavailable`。审计 `experiments/m9_1_runner.py` 确认原因是计量字典均以全量 `metric_defaults()` 初始化，最终 `record` 合并时后续的 StateVector 或 Memory 字典会覆盖先前已经计算出的 Protocol 或 StateVector 数值。这不是可按缺失字段解释的正常观测限制，而是正式 Runner 的计量合并框架错误，导致 M9.1 的核心通信效率、状态压缩和记忆门控研究问题无法由本次记录回答。

因此 P9.11 工程完整性和公开泄漏门槛通过，但指标完整性验收失败；这 240 条产物必须保留供审计，不得删除、篡改或选择性重跑，也不得作为有效 M9.1 正式结果写入报告或发布。当前唯一 freeze `87c4461` 不再可用于有效 M9.1 正式实验。后续修复必须先在合成 fixtures 和 Mock Backend 上验证逐类计量合并、双平台回归和公开 schema，再重新生成 Spec、implementation SHA、freeze SHA、两项真实 canary 和一次新的完整 240 条正式运行；不得从本次失败结果调参、换题或修改统计方法。

P9.11 不允许进入结果聚合、Dashboard、技术报告或发布阶段。唯一下一步是 P9.12：修复独立 M9.1 Runner 的指标合并逻辑，并以合成 fixtures 证明 Protocol、StateVector、Memory 和模型指标不会互相覆盖；修复验收通过前不得再次运行真实模型。

## M9.1 P9.12 指标合并修复

P9.12 修复了 `experiments/m9_1_runner.py` 中的指标合并错误。公开记录仍先由一次 `metric_defaults()` 建立稳定 schema；Protocol、StateVector、SharedMemory 和模型推理四个来源改为从空字典开始，仅写入本来源实际测得的字段，因此后续来源不再以 `unavailable` 覆盖先前已测通信、状态或记忆指标。无法可靠恢复的字段仍保持 `unavailable`，未用 `0` 或推测值替代。

新增 Mock Backend 回归测试完整执行 S4 的公开计量路径，断言通信、StateVector、Memory 和模型用量字段可同时保留。Windows 专项测试为 `7 passed`，全量测试为 `96 passed`，`git diff --check` 通过；本阶段未调用真实模型，未读取或公开 private tests，未改动 M9 正式结果、freeze SHA `cc7aac0417afb6acab47baaf7449459692fa9444`、`reports/m9`、Dashboard 或 `v1.0.0-experiment`。

首次 M9.1 运行所用 freeze `87c4461` 及其 240 条记录仍永久保留为失效计量运行，不能作为有效 M9.1 结果。P9.12 仅完成 Windows 修复验收，尚未完成 openEuler 同步和双平台复核，尚不允许生成新 Spec、freeze、canary 或再次启动正式实验。

唯一下一步是：将 P9.12 修复同步到 openEuler，在不调用真实模型的条件下完成专项、全量和 Mock 公开计量路径验收。

## M9.1 P9.12 双平台验收完成

P9.12 修复提交为 `47c6647254cce1419b3ea0e2438db0039570ceee`。openEuler 主工作树以 Git bundle 执行 `git fetch` 后的 `git merge --ff-only` 同步到该 SHA；同步未接触 `87c4461` 的 freeze worktree、其失效的 240 条运行产物或任何 M9 目录。openEuler 专项测试为 `7 passed`，全量测试为 `95 passed, 1 skipped`，`git diff --check` 通过且工作树干净。专项 S4 Mock 回归覆盖完整公开计量链，断言通信、StateVector、Memory 和模型用量字段均不会保留为 `unavailable`。
...
P9.15 在 Windows 的 `H:\OA\LiteTest-MAS-m9_1-freeze-c79` 与 openEuler 的 `<openEuler 工作目录>/LiteTest-MAS-m9_1-freeze-c79` 创建 detached worktree，两端均固定为候选 freeze `c79fd4826627bf61faf5d90540a014d243a59edd`。旧 M9.1 worktree `<openEuler 工作目录>/LiteTest-MAS-m9_1-freeze-7c7` 继续保留，M9 freeze worktree、M9 公开结果、Dashboard 与发布标签均未触碰。
...
P9.17 已在 openEuler 的独立 detached worktree `<openEuler 工作目录>/LiteTest-MAS-m9_1-freeze-c79` 完成一次 M9.1 正式 240 条运行。运行固定在 freeze SHA `c79fd4826627bf61faf5d90540a014d243a59edd`，使用确定性 Spec SHA `3ad520c75bb66e8a4617daa64d6824183cbeaa5a1e1cb01dcd50035f145231f6`；未改变任务、顺序、模型、组件、生成参数、评测边界或统计方法。
...
P9.18 首次公开聚合审计发现：正式记录中的 Memory 计数是 sequence 累计值，旧聚合器直接按任务求平均会失真；dataset、task-group 和 seed 汇总也必须保留 `experiment_group` 维度，不能混合 S1-S4。修正后的聚合器按 `plan_index` 将累计 Memory 计数还原为逐任务增量，并从计数总和重算可定义比率。Runner 在建立 Protocol 指标差分基线前已生成握手，因此正式记录中的握手零值不能解释为没有握手，聚合统一标记为 `unavailable`。该修正只处理公开记录，不修改 freeze、Spec 或正式结果。
